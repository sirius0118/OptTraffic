...
Service 
        ...
        print("图为", self.graphSet.GraphSet.keys())
        for graph in self.graphSet.GraphSet.keys():
...
service对流量已经低于Threshold还没有找到一个需要本地化的边， 
            # if i < self.Threshold:
            #     if len(result) == 0:
            #         return False
            #     else:
...
Service是否已经进行了本地化，若没有则进行本地化。若已经进行了本地化， 
...
graph—service中， 
    ...
    def Localize_i(self, result: str):
        """
        本函数用于将指定的Service-Service 
         
            1v1： 
            1vn：能将几个放一起就将几个放一起，可以同时考虑所有的链路，拟合速度比NetMARKS 
            nvn： 
        :return:
        """

        # TODO:迁移过程中，可能上游微服务或者下游微服务已经进行了本地化， 
        #   
        def update(result):
...
choice之后就需要将所有上下游的Pod从Freedom到Matched
...
solo list为长度为1的列表，more list 为长度大于1 
            if len(dm_list) == 1:
                solo_list = dm_list
                more_list = um_list
            else:
...
Stateful无法进行hotExchange
            if more_list[0].Stateful and solo_list[0].Stateful:
                update(result)
                return False
             
service是否之前已经参与过本地化。若都没参加过，或者只有solo参加过，或则solo是stateful
